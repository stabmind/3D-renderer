...
void Renderer::Render(const World &world, const Camera &camera,
  ...
  for (const auto &it : world) {
    ...
    MakeViewTransformation(camera, &triangle);
    ...
    for (const auto &it : triangles) {
      Triangle triangle = it;
      MakeProjectionTransformation(camera, &triangle);
...
void Renderer::MakeViewTransformation(const Camera &camera,
...
void Renderer::MakeProjectionTransformation(const Camera &camera,
                                            Triangle *p_triangle) const {
  assert(p_triangle != nullptr && "p_triangle must not be nullptr");
...
void Renderer::Clip(const Triangle &triangle, const PointVec_2d &poly_2d,
  ...
  const Vector3d &tp1 = triangle.getPoint(0);
  const Vector3d &tp2 = triangle.getPoint(1);
  const Vector3d &tp3 = triangle.getPoint(2);
  ...
  for (auto it : poly_selected) {
    ...
    vertices.push_back({Vector3d(x, y, z), c});
  }
...
void Renderer::FrustumClip_xz(const Camera &camera, const Triangle &triangle,
  ...
  auto ReversePoint_xzy = [](const Vector3d &p) {
    return Vector3d(p[0], p[2], p[1]);
  };
  ...
  for (const auto &it : triangles_xzy) {
    const Vector3d &tp1 = it.getPoint(0);
    const Vector3d &tp2 = it.getPoint(1);
    const Vector3d &tp3 = it.getPoint(2);
...
void Renderer::FrustumClip_yz(const Camera &camera, const Triangle &triangle,
  ...
  auto ReversePoint_yzx = [](const Vector3d &p) {
    return Vector3d(p[1], p[2], p[0]);
  };

  auto UnreversePoint_yzx = [](const Vector3d &p) {
    return Vector3d(p[2], p[0], p[1]);
  };
...
void Renderer::IntersectPolygons_2d(const Triangle &triangle,
  ...
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = 0; j < m; ++j) {
      const Vector3d &tp1 = triangle.getPoint(i);
      const Vector3d &tp2 = triangle.getPoint((i + 1) % n);
      ...
      if (intersectSegments(v1, v2, u1, u2, &intersection_point) &&
        ...
        p_vertices->push_back({Vector3d(x, y, z), c});
      }
    }
  }
}
...
void Renderer::Triangulate(VertexVec vertices,
  ...
  Vector3d start_v = vertices[0].point;
  ...
  for (size_t i = 1; i + 1 < n; ++i) {
    const Vector3d &p1 = vertices[0].point;
    const Vector3d &p2 = vertices[i].point;
    const Vector3d &p3 = vertices[i + 1].point;
  ...
  for (size_t i = 0; i < dim; ++i) {
    for (size_t j = i + 1; j < dim; ++j) {
      for (size_t k = 1; k + 1 < n; ++k) {
        const Vector3d &v = vertices[0].point - vertices[k].point;
        const Vector3d &u = vertices[0].point - vertices[k + 1].point;
...
void Renderer::getScreenCoords(const Vector3d &p, ScreenCoordsType *x,
                               ScreenCoordsType *y, SpaceCoordsType *z) const {
  Vector3d fit_p = FitPoint(p);
...
Vector3d Renderer::FitPoint(const Vector3d &p) const {
  Vector3d::Scalar fit_x = (p[0] + 1) / 2 * w_;
  Vector3d::Scalar fit_y = (1 - (p[1] + 1) / 2) * h_;
  return Vector3d(fit_x, fit_y, p[2]);
}
