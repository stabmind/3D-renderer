...
class Triangle {
 public:
  ...
  // clang-format off
  Triangle(const Vector3d &p1, ColorType c1,
  ...
  Vector3d getPoint(IndexType i) const;
  ColorType getColor(IndexType i) const;

  std::array<Vector4d, 3>::iterator begin();
  std::array<Vector4d, 3>::const_iterator begin() const;
  std::array<Vector4d, 3>::iterator end();
  std::array<Vector4d, 3>::const_iterator end() const;

 private:
  std::array<Vector4d, 3> hpoints_;
